...
inline std::string extract_any(const std::any & value,
					 const std::int64_t value_type_hash = static_cast<std::int64_t>(typeid(std::size_t).hash_code())) {
...
inline Metadata extract_metadata(const dictionary & parsed_content, const std::string & file_content = "") noexcept {
	...
	for(const auto & [dict_key, value] : parsed_content) {
		...
		if(dict_key == "creation date") {
			...
			std::strftime(metadata.creation_time.data(), metadata.creation_time.size(), "%H:%M:%S - %a - %d %b %Y",
